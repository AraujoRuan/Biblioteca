...
<head>
    ...
    <title>Adicionar Livro - BookMaster</title>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center py-4">
                ...
                <nav class="hidden md:flex space-x-8">
                    ...
                    <a href="{{ route('books.create') }}" class="text-blue-600 font-medium border-b-2 border-blue-600">Adicionar Livro</a>
                </nav>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <select id="category" name="category" required
                            ...
                            <option value="Ficção" {{ old('category') == 'Ficção' ? 'selected' : '' }}>Ficção</option>
                            <option value="Romance" {{ old('category') == 'Romance' ? 'selected' : '' }}>Romance</option>
                            <option value="Aventura" {{ old('category') == 'Aventura' ? 'selected' : '' }}>Aventura</option>
                            <option value="Tecnologia" {{ old('category') == 'Tecnologia' ? 'selected' : '' }}>Tecnologia</option>
                            <option value="Biografia" {{ old('category') == 'Biografia' ? 'selected' : '' }}>Biografia</option>
                            <option value="História" {{ old('category') == 'História' ? 'selected' : '' }}>História</option>
                            <option value="Ciência" {{ old('category') == 'Ciência' ? 'selected' : '' }}>Ciência</option>
                            <option value="Fantasia" {{ old('category') == 'Fantasia' ? 'selected' : '' }}>Fantasia</option>
                            <option value="Infantil" {{ old('category') == 'Infantil' ? 'selected' : '' }}>Infantil</option>
                            <option value="Autoajuda" {{ old('category') == 'Autoajuda' ? 'selected' : '' }}>Autoajuda</option>
                        </select>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="file" id="cover_image" name="cover_image"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200"
                               accept="image/*">
                        <p class="mt-1 text-sm text-gray-500">Formatos: JPG, PNG, GIF. Tamanho máximo: 2MB</p>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="description" name="description" rows="4"
                                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200"
                                  placeholder="Descrição breve do livro...">{{ old('description') }}</textarea>
                    </div>
                </div>
